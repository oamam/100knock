...
def predict(lr, features):
    sentences = [input('sentence > ')]

    X = explanatory_variables(sentences, features)
    if lr.predict(X)[0] == 1:
        print('predict: {}'.format('+1'))
        print('probability: {}'.format(probability(X[0], lr)))
    else:
        print('predict: {}'.format('-1'))
        print('probability: {}'.format(1 - probability(X[0], lr)))
