...
def main():
    word = input('word > ')
    print(has_stop_word(word))


def has_stop_word(word):
    stop_words = (
...
class TestStopWord(unittest.TestCase):
    def test_has_stop_word(self):
        for word in (
            ...
            actual = has_stop_word(word)
            expected = True
            self.assertEqual(expected, actual)

    def test_not_has_stop_word(self):
        for word in ['fsa', 'liei', 'l', 'due']:
            actual = has_stop_word(word)
            expected = False
            self.assertEqual(expected, actual)
